...
public class AddSize extends HttpServlet {
    ...
    throws ServletException, IOException {
       String memorySizeStr = request.getParameter("memorySize");
        int memorySize = Integer.parseInt(memorySizeStr);

        Size size = new Size();
        size.setMemorySize(memorySize);

        SizeDAO sizeDAO = new SizeDAO();
        int result = sizeDAO.insert(size);

        if (result > 0) {
            response.sendRedirect("attributes");
        } else {
            // Thất bại, quay lại với thông báo lỗi
            request.setAttribute("error", "Error while inserting size.");
